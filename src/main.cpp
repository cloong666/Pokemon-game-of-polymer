...
void loadPythonSkill(std::shared_ptr<Move> move, const std::string& scriptName) {
    try {
        ...
        std::cout << "[OK] Loaded " << move->getName() << " skill from Python script." << std::endl;
    } catch (const std::exception& e) {
        std::cout << "Using default effect for " << move->getName()
                  << " (failed to load '" << scriptName << "': " << e.what() << ")"
                  << std::endl;
    }
}
...
int main() {
    ...
    try {
        ...
        std::cout << "\n[OK] All Pokemon and moves created successfully!\n" << std::endl;
